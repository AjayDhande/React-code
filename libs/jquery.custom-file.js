...
(function($) {
    var CustomFile = function(el, opts) {
        ...
        var settings = $.extend({}, $.fn.customFile.defaults, opts),
          ...
          button = $('<span class="' + klass + ' button choose" aria-hidden="true">' + settings.text + '</span>').appendTo(upload),
          //create custom control feedback
          feedback = $('<span class="' + klass + '-feedback" aria-hidden="true">'+settings.pretext+'</span>');
        if (settings.status) {
        ...
        $el.addClass(klass + '-input')
            ...
            .bind('change',function(){
                // Return if there's no status to update
                if (!settings.status) {
    ...
    // default settings
    $.fn.customFile.defaults = { status:true, text: 'Choose File', selected: 'File Selected' };
})(jQuery);
